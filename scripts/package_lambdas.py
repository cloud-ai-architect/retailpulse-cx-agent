#!/usr/bin/env python
"""Build and deploy the RetailPulse Lambda package.
...
def build(staging: pathlib.Path) -> bytes:
    ...
    buf = pathlib.Path(tempfile.gettempdir()) / "retailpulse-lambda.zip"
...
def main() -> int:
    ...
    if args.dry_run:
        out = pathlib.Path(tempfile.gettempdir()) / "retailpulse-lambda.zip"
        print("dry-run: %s" % out)
        return 0
