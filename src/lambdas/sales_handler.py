...
@stage(name="sales-agent", input_model=OrchestratorDecision, output_model=AgentResponse)
class SalesAgentLambda(BaseLambda):
    def setup(self) -> None:
        pass
    ...
    def handle(self, ctx: JobContext, inp: OrchestratorDecision) -> AgentResponse:  # type: ignore[override]
        ...
        try:
            ...
            response_text = run_sales_agent(customer_id, transcript, inp.context_for_agent)
            ...
            return AgentResponse(
                ...
                intent="sales",
                agent="sales",
                response=f"I'm having trouble looking that up right now. Could you try again?",
